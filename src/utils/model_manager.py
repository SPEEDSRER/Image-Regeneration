...
class ModelManager:
    ...
    def set_llm_model(self, model_type: str, **kwargs):
        ...
        if model_type == "openai":
            ...
            if "model_path" not in kwargs:
                raise ValueError("Model path is required for Qwen")
            self._load_qwen(kwargs["model_path"])
    ...
    def set_vision_models(self):
        ...
        if self.clip_model is None:
            ...
            self.clip_processor = CLIPProcessor.from_pretrained("../models/clip")
            self.clip_model = CLIPModel.from_pretrained("../models/clip").to(self.device)
        
        if self.dinov2_model is None:
            print("Loading DINOv2 model...")
            self.dinov2_processor = AutoImageProcessor.from_pretrained("../models/dinov2")
            self.dinov2_model = AutoModel.from_pretrained("../models/dinov2").to(self.device)

    def _load_qwen(self, model_path):
    ...
    def get_diffusion_model(self) -> StableDiffusionXLPipeline:
